    ...
    private static void Cenario_1()
    {
        ...
        private static void Cenario_6()
        {
            Policy.Handle<WebException>()
                ...
                {                    
                    Debug.WriteLine($"Tipo de exceção: {error.GetType().Name}, executado depois de: {tempo}. Horário: {DateTime.Now}");
                })
                .Execute(() =>
                {                    
                    using (var webclient = new WebClient())
        ...
        private static void Cenario_6B()
        {
            Policy.Handle<WebException>()
                .CircuitBreaker(3, TimeSpan.FromSeconds(3), (error, tempo) => { }, () =>
                {
                    Executar();
                })
                .Execute(() =>
